      ...
      <table className="table table-striped table-hover">
        ...
        <tbody>
          {books.length ? books.map(book => (
            <tr key={book._id} onClick={() => onRowClick(book._id)} style={{cursor:'pointer'}}>
              <td dangerouslySetInnerHTML={{__html: highlightText(book.title, searchQuery)}}/>
              <td dangerouslySetInnerHTML={{__html: highlightText(book.series||'', searchQuery)}}/>
              <td dangerouslySetInnerHTML={{__html: highlightText(book.author.join(', '), searchQuery)}}/>
            ...
            <tr><td colSpan="7">No books match the search criteria.</td></tr>
          )}
        </tbody>
      </table>
    </div>        
